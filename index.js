...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} ja era pra ter saido kkkk 👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Espera sapouha um pouco... ⌛',
				success: '✔️ Pronto fi, cai fora agora kk ✔️',
				error: {
				...
				only: {
					...
					ownerG: '⚠️ Este comando só pode ser usado pelo dono do bot trouxa kkkk! ',
					ownerB: '❌ Este comando só pode ser usado pelo proprietário do bot! ❌',
					admin: '⚠️ Este comando só pode ser usado por admins bb! ',
					Badmin: '❌ Este comando só pode ser usado quando o bot se torna um administrador! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5591999830374@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Adm proibiu porno no gp seu punhetero🙄'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/d1M6lOz.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Sixxx\n*YOUTUBE:* to sem\n*WPP:* wa.me/+5591999830374\n*INSTA:* to sem seu corno kkkk\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'hentai':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'cara bate pra 2d kkkk'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do sixxx:*\n\n https://bit.ly/3omUNCg'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heher boy🙉 se fudeu kkk'})
					})
					break
				case 'nsfwloli':
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('Comando so funfa, se você for o dono do bot seu animal 😡')
        ...
        case 'promote':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('1 para ativar, 0 para desativar, lerdão você em pqp mlk burro 🤦')
					}
					break
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
